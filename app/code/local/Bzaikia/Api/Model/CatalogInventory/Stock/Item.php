 ...
 */
class Bzaikia_Api_Model_CatalogInventory_Stock_Item extends Mage_CatalogInventory_Model_Stock_Item
{
    /**
    ...
    public function checkQuoteItemQty($qty, $summaryQty, $origQty = 0)
    {
        ...
        if (!$this->getIsInStock()) {
            $result->setHasError(true)
                ->setMessage(Mage::helper('cataloginventory')->__('Product "%s" is currently out of stock.', $this->getProduct()->getSku()))
        ...
        if (!$this->checkQty($summaryQty) || !$this->checkQty($qty)) {
            $message = Mage::helper('cataloginventory')->__('The requested quantity for "%s" is not available. Quantity remaining  is %d', $this->getProduct()->getSku(), $this->getQty());
            $result->setHasError(true)
                ->setMessage($message)
                ->setQuoteMessage($message)
                ->setQuoteMessageIndex('qty');
            $result->setData('keep_going', 1);
            return $result;
        } else {
